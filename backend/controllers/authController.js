...
exports.register = async (req, res) => {
  ...
  console.log(`[Auth Ctrl] Register attempt: ${email}`);
  ...
  try {
    ...
    const insertQuery = `INSERT INTO users (email, password, first_name, last_name, is_premium) VALUES ($1, $2, $3, $4, FALSE) RETURNING user_id, email, first_name, last_name, is_premium;`;
    const values = [
      email.toLowerCase(),
      hashedPassword,
      firstName,
    ...
    console.log(
      `[Auth Ctrl] Register success: ${newUser.email}, ID: ${newUser.user_id}`
    );
    res
      .status(201)
...
exports.login = async (req, res) => {
  ...
  try {
    const findUserQuery =
      "SELECT user_id, email, password, first_name, last_name, is_premium FROM users WHERE email = $1"; // Added last_name
    ...
    console.log(
      `[Auth Ctrl] Login success: ${user.email}, ID: ${user.user_id}`
    );
    res
      .status(200)
      .json({

...
const DiaryPage = () => {
  ...
  const fetchDiaryEntry = useCallback(async () => {
    ...
    console.log(
      `[DiaryPage] Fetching entry for date: ${selectedDate}, User: ${userInfo.appUserId}`
    );
    setIsLoading(true);
    setError("");
    setSuccessMessage("");
    ...
    try {
      ...
      console.error(
        "[DiaryPage] Error fetching diary entry:",
  ...
  const handleSaveEntry = async (e) => {
    ...
    try {
      ...
      console.error(
        "[DiaryPage] Error saving diary entry:",
        err.response?.data || err.message
      );
      setError(
  ...
  const handleDeleteEntry = async () => {
    ...
    console.log(
      `[DiaryPage] Deleting entry for date: ${selectedDate}, ID: ${currentEntryId}`
    );
    ...
    try {
      await apiClient.delete("/api/diary", {
        params: { date: selectedDate }, // Auth header added by interceptor
      });
      ...
      console.error(
        "[DiaryPage] Error deleting diary entry:",
        err.response?.data || err.message
      );
      setError(
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "700px", margin: "auto" }}>
      ...
      {isLoading && (
        <p style={{ fontStyle: "italic", color: "#555" }}>Loading entry...</p>
      )}
